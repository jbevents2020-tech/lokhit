...
-- Public bucket for newsroom featured images. Objects are namespaced by user id.
...
drop policy if exists "news_images_storage_insert" on storage.objects;
create policy "news_images_storage_insert" on storage.objects
for insert to authenticated
with check (
...
drop policy if exists "news_images_storage_update" on storage.objects;
create policy "news_images_storage_update" on storage.objects
for update to authenticated
using (
  bucket_id = 'news-images'
...
drop policy if exists "news_images_storage_delete" on storage.objects;
create policy "news_images_storage_delete" on storage.objects
for delete to authenticated
using (
  bucket_id = 'news-images'
  and ((storage.foldername(name))[1] = auth.uid()::text or public.current_user_role() in ('admin', 'editor'))
