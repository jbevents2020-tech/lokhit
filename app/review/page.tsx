...
import { ArrowLeft, CheckCircle2, Clock3, LoaderCircle, RefreshCw, XCircle } from "lucide-react";
...
export default function ReviewPage() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("news")
      .select("id, title, excerpt, content, location, status, created_at, author:profiles!news_author_id_fkey(full_name), category:categories(name)")
  ...
  async function approve(item: ReviewItem) {
    ...
    else { setItems((current) => current.filter((news) => news.id !== item.id)); setNotice({ type: "success", text: `“${item.title}” approve झाली.` }); }
    setWorkingId(null);
  }
