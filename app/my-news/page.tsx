...
type NewsItem = { id: string; title: string; excerpt: string | null; featured_image_url: string | null; status: NewsStatus; rejection_reason: string | null; created_at: string; updated_at: string; category: { name: string } | null };
...
export default function MyNewsPage() {
  ...
  const loadNews = useCallback(async (authorId: string) => {
    ...
    const { data, error: queryError } = await supabase.from("news").select("id, title, excerpt, featured_image_url, status, rejection_reason, created_at, updated_at, category:categories(name)").eq("author_id", authorId).order("updated_at", { ascending: false });
